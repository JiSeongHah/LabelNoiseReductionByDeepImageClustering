...
from simple_torch2 import simple_torch2
from save_funcs import load_my_model


class excute_simple_rl2:
    def __init__(self,
                 gamma,
                 eps,
                 rl_lr,
    ...
    def excute_RL(self):
        ...
        RL_train_data_rest = torch.from_numpy(RL_train_data[RL_train_label != 0]).unsqueeze(1)
        ...
        RL_val_ones = torch.ones_like(RL_val_labels)
        RL_val_labels = torch.where(RL_val_labels >=1, RL_val_ones,RL_val_labels)
        ...
        try:
            REINFORCE_START = torch.load(self.model_save_load_path + self.modelLoadNum + '.pth')
            print('successsuccesssuccesssuccesssuccesssuccesssuccesssuccess')
        except:
            print('model loading failed so loaded fresh model')
            REINFORCE_START = simple_torch2(gamma=self.gamma,
                                            eps=self.eps,
                                            rl_lr=self.rl_lr,
        ...
        for i in range(10000000):
            ...
            REINFORCE_START.STARTTRNANDVAL(RL_td_zero=RL_train_data_zero_little,
                                           RL_tl_zero=RL_train_label_zero_little,
                                           RL_td_rest=RL_train_data_rest,
                                           RL_tl_rest=RL_train_label_rest,
                                           i=i,
                                           RL_valInput= RL_val_inputs,
                                           RL_valLabel= RL_val_labels)

            if i % self.RL_save_range == 0 and i != 0:
                try:
                    torch.save(REINFORCE_START,self.model_save_load_path + str(i) + '.pth')
                    print('saving RL model complete')

                except:
                    print('saving model failed')
            print(f'{i} th epoch for RL complete')
            print('                   ')
