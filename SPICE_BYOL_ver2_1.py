    ...
    def trainHeadOnly(self):
        ...
        for h in range(self.numHeads):
            ...
            for eachCluster in range(self.clusterNum):
                ...
                sumedTensor = torch.mean(eachSelectedTensor, dim=0)
        ...
        for eachHeadIdx in range(self.numHeads):
            print(f'filtered input size per head {eachHeadIdx} is :{PseudoInputPerHead[h].size()}'
                  f' and label is : {PseudoLabelPerHead[h].size()}')
        ...
        for eachRepeat in range(self.numRepeat):
            ...
            print(f'innerRepeatNum is {innerRepeatNum}')
            ...
            print(f'training {eachRepeat}th/{self.numRepeat}th inner training complete'
                  f' with loss : {lossMean.item()}')
